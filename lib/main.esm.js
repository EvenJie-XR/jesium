import { Cartesian3, Math as Math$1, ScreenSpaceEventHandler, ScreenSpaceEventType, defined, CallbackProperty, PolygonHierarchy, Cartographic, BillboardCollection, Cesium3DTileset, Cesium3DTileStyle, Viewer, ArcGisMapServerImageryProvider, Color } from 'cesium';
...
class CameraUtils {
    ...
    flyToByCoordArray(coord) {
        this.viewer.camera.flyTo({
            destination: Cartesian3.fromDegrees(...coord)
        });
    }
    /**
    ...
    getCameraInfo() {
        ...
        return {
            latitude: Math$1.toDegrees(catoPosition.latitude),
            longitude: Math$1.toDegrees(catoPosition.longitude),
    ...
    addCompass(compassEl) {
        return this.addCameraChangeEvent(() => {
            const rotateAngle = Math$1.toDegrees(this.viewer.camera.heading).toFixed(2);
    ...
    resetCameraHeading() {
        ...
        this.viewer.camera.flyTo({
            destination: Cartesian3.fromDegrees(cameraInfo.longitude, cameraInfo.latitude, cameraInfo.height),
            orientation: {
                heading: 0,
                pitch: Math$1.toRadians(cameraInfo.pitch),
                roll: Math$1.toRadians(cameraInfo.roll)
            }
        });
    }
}
        ...
        this.__temporary_polygon_entity = this.viewer.entities.add({
            polygon: {
                ...
                hierarchy: new CallbackProperty(() => {
                    ...
                    return new PolygonHierarchy(this.__polygon_point_arr);
                }, false),
                material: polygonColor.withAlpha(0.22),
            },
            polyline: {
...
class CoordUtils {
    ...
    cato2Lat(cato) {
        return {
            latitude: Math$1.toDegrees(cato.latitude),
            longitude: Math$1.toDegrees(cato.longitude),
            height: cato.height
        };
    }
    /**
     ...
     */
    lat2Cato(coord) {
        const cartographic = Cartographic.fromDegrees(...coord);
        return cartographic;
    }
    /**
     * Cartographic转笛卡尔3
    ...
    set3DTilesOpacityByUUID(uuid, opacity) {
        ...
        if (threeDTiles) {
            if (opacity == 0) {
                ...
                threeDTiles.style = new Cesium3DTileStyle({
...
export { CameraUtils, ControlUtils, CoordUtils, ImageryUtils, Jesium, ModelUtils, version };
